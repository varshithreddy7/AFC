import React from "react";
import Image from "next/image";

export const Frame = () => {
  return (
    <div className="absolute w-[1090px] h-[1696px] top-[1465px] left-[871px] overflow-hidden overflow-y-scroll">
      <div className="absolute w-[1078px] h-[382px] top-0 left-0 bg-black">
        <div className="relative w-[1063px] h-[382px]">
          <div className="absolute w-[1047px] h-[382px] top-0 left-0">
            <Image
              className="w-[573px] h-[382px] top-0 left-0 absolute object-cover"
              alt="Image"
              src="https://c.animaapp.com/89fS0TNm/img/image-64-4.png"
              width={573}
              height={382}
            />

            <div className="absolute w-[292px] top-[68px] left-[562px] [font-family:'Nunito',Helvetica] font-black text-white text-4xl tracking-[0] leading-10 whitespace-nowrap">
              Premium Burger
            </div>

            <div className="absolute w-[152px] h-6 top-[143px] left-[562px]">
              <Image
                className="absolute w-6 h-6 top-0 left-0"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-8"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-16"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-24"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-32"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-14.svg"
                width={24}
                height={24}
              />
            </div>

            <p className="absolute w-[485px] top-[191px] left-[562px] [font-family:'Nunito',Helvetica] font-normal text-white text-lg tracking-[0] leading-[29.2px]">
              Experience the perfect blend of premium ingredients and expert
              craftsmanship in every bite.
            </p>

            <p className="absolute w-[105px] top-[263px] left-[559px] [font-family:'Inter',Helvetica] font-normal text-[#ebbd28] text-3xl tracking-[0] leading-9 whitespace-nowrap">
              <span className="font-black">₹19.99</span>
            </p>
          </div>

          <div className="absolute w-[156px] h-[60px] top-[252px] left-[907px] bg-[#ebbd28] rounded-[26843500px]">
            <div className="absolute w-[95px] top-[15px] left-8 [font-family:'Nunito',Helvetica] font-bold text-black text-lg text-center tracking-[0] leading-7 whitespace-nowrap">
              Order Now
            </div>
          </div>

          <div className="absolute w-[243px] h-[11px] top-[114px] left-[574px]">
            <div className="absolute w-3 h-[11px] top-0 left-[115px] bg-[#ebbd28] rounded-[5.96px/5.5px]" />

            <Image
              className="left-0 absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-1.svg"
              width={95}
              height={1}
            />

            <Image
              className="left-[148px] absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-2.svg"
              width={95}
              height={1}
            />
          </div>
        </div>
      </div>

      <div className="absolute w-[1078px] h-[382px] top-[1783px] left-[9px] bg-black">
        <div className="relative w-[1063px] h-[382px]">
          <div className="absolute w-[1047px] h-[382px] top-0 left-0">
            <Image
              className="w-[573px] h-[382px] top-0 left-0 absolute object-cover"
              alt="Image"
              src="https://c.animaapp.com/89fS0TNm/img/image-64-4.png"
              width={573}
              height={382}
            />

            <div className="absolute w-[292px] top-[68px] left-[562px] [font-family:'Nunito',Helvetica] font-black text-white text-4xl tracking-[0] leading-10 whitespace-nowrap">
              Premium Burger
            </div>

            <div className="absolute w-[152px] h-6 top-[143px] left-[562px]">
              <Image
                className="absolute w-6 h-6 top-0 left-0"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-8"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-16"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-24"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-32"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-24.svg"
                width={24}
                height={24}
              />
            </div>

            <p className="absolute w-[485px] top-[191px] left-[562px] [font-family:'Nunito',Helvetica] font-normal text-white text-lg tracking-[0] leading-[29.2px]">
              Experience the perfect blend of premium ingredients and expert
              craftsmanship in every bite.
            </p>

            <p className="absolute w-[105px] top-[263px] left-[559px] [font-family:'Inter',Helvetica] font-normal text-[#ebbd28] text-3xl tracking-[0] leading-9 whitespace-nowrap">
              <span className="font-black">₹19.99</span>
            </p>
          </div>

          <div className="absolute w-[156px] h-[60px] top-[252px] left-[907px] bg-[#ebbd28] rounded-[26843500px]">
            <div className="absolute w-[95px] top-[15px] left-8 [font-family:'Nunito',Helvetica] font-bold text-black text-lg text-center tracking-[0] leading-7 whitespace-nowrap">
              Order Now
            </div>
          </div>

          <div className="absolute w-[243px] h-[11px] top-[114px] left-[574px]">
            <div className="absolute w-3 h-[11px] top-0 left-[115px] bg-[#ebbd28] rounded-[5.96px/5.5px]" />

            <Image
              className="left-0 absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-1-4.svg"
              width={95}
              height={1}
            />

            <Image
              className="left-[148px] absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-2-5.svg"
              width={95}
              height={1}
            />
          </div>
        </div>
      </div>

      <div className="absolute w-[1078px] h-[382px] top-[438px] left-[3px] bg-black">
        <div className="relative w-[1063px] h-[382px]">
          <div className="absolute w-[1047px] h-[382px] top-0 left-0">
            <Image
              className="w-[573px] h-[382px] top-0 left-0 absolute object-cover"
              alt="Image"
              src="https://c.animaapp.com/89fS0TNm/img/image-64-4.png"
              width={573}
              height={382}
            />

            <div className="absolute w-[292px] top-[68px] left-[562px] [font-family:'Nunito',Helvetica] font-black text-white text-4xl tracking-[0] leading-10 whitespace-nowrap">
              Premium Burger
            </div>

            <div className="absolute w-[152px] h-6 top-[143px] left-[562px]">
              <Image
                className="absolute w-6 h-6 top-0 left-0"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-8"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-16"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-24"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-13.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-32"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-14.svg"
                width={24}
                height={24}
              />
            </div>

            <p className="absolute w-[485px] top-[191px] left-[562px] [font-family:'Nunito',Helvetica] font-normal text-white text-lg tracking-[0] leading-[29.2px]">
              Experience the perfect blend of premium ingredients and expert
              craftsmanship in every bite.
            </p>

            <p className="absolute w-[105px] top-[263px] left-[559px] [font-family:'Inter',Helvetica] font-normal text-[#ebbd28] text-3xl tracking-[0] leading-9 whitespace-nowrap">
              <span className="font-black">₹19.99</span>
            </p>
          </div>

          <div className="absolute w-[156px] h-[60px] top-[252px] left-[907px] bg-[#ebbd28] rounded-[26843500px]">
            <div className="absolute w-[95px] top-[15px] left-8 [font-family:'Nunito',Helvetica] font-bold text-black text-lg text-center tracking-[0] leading-7 whitespace-nowrap">
              Order Now
            </div>
          </div>

          <div className="absolute w-[243px] h-[11px] top-[114px] left-[574px]">
            <div className="absolute w-3 h-[11px] top-0 left-[115px] bg-[#ebbd28] rounded-[5.96px/5.5px]" />

            <Image
              className="left-0 absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/image.svg"
              width={95}
              height={1}
            />

            <Image
              className="left-[148px] absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-2-2.svg"
              width={95}
              height={1}
            />
          </div>
        </div>
      </div>

      <div className="absolute w-[1078px] h-[382px] top-[876px] left-1.5 bg-black">
        <div className="relative w-[1063px] h-[382px]">
          <div className="absolute w-[1047px] h-[382px] top-0 left-0">
            <Image
              className="w-[573px] h-[382px] top-0 left-0 absolute object-cover"
              alt="Image"
              src="https://c.animaapp.com/89fS0TNm/img/image-64-4.png"
              width={573}
              height={382}
            />

            <div className="absolute w-[292px] top-[68px] left-[562px] [font-family:'Nunito',Helvetica] font-black text-white text-4xl tracking-[0] leading-10 whitespace-nowrap">
              Premium Burger
            </div>

            <div className="absolute w-[152px] h-6 top-[143px] left-[562px]">
              <Image
                className="absolute w-6 h-6 top-0 left-0"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-18.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-8"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-18.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-16"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-18.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-24"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-18.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-32"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-19.svg"
                width={24}
                height={24}
              />
            </div>

            <p className="absolute w-[485px] top-[191px] left-[562px] [font-family:'Nunito',Helvetica] font-normal text-white text-lg tracking-[0] leading-[29.2px]">
              Experience the perfect blend of premium ingredients and expert
              craftsmanship in every bite.
            </p>

            <p className="absolute w-[105px] top-[263px] left-[559px] [font-family:'Inter',Helvetica] font-normal text-[#ebbd28] text-3xl tracking-[0] leading-9 whitespace-nowrap">
              <span className="font-black">₹19.99</span>
            </p>
          </div>

          <div className="absolute w-[156px] h-[60px] top-[252px] left-[907px] bg-[#ebbd28] rounded-[26843500px]">
            <div className="absolute w-[95px] top-[15px] left-8 [font-family:'Nunito',Helvetica] font-bold text-black text-lg text-center tracking-[0] leading-7 whitespace-nowrap">
              Order Now
            </div>
          </div>

          <div className="absolute w-[243px] h-[11px] top-[114px] left-[574px]">
            <div className="absolute w-3 h-[11px] top-0 left-[115px] bg-[#ebbd28] rounded-[5.96px/5.5px]" />

            <Image
              className="left-0 absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-1-2.svg"
              width={95}
              height={1}
            />

            <Image
              className="left-[148px] absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-2-3.svg"
              width={95}
              height={1}
            />
          </div>
        </div>
      </div>

      <div className="absolute w-[1078px] h-[382px] top-[1314px] left-3 bg-black">
        <div className="relative w-[1063px] h-[382px]">
          <div className="absolute w-[1047px] h-[382px] top-0 left-0">
            <Image
              className="w-[573px] h-[382px] top-0 left-0 absolute object-cover"
              alt="Image"
              src="https://c.animaapp.com/89fS0TNm/img/image-64-4.png"
              width={573}
              height={382}
            />

            <div className="absolute w-[292px] top-[68px] left-[562px] [font-family:'Nunito',Helvetica] font-black text-white text-4xl tracking-[0] leading-10 whitespace-nowrap">
              Premium Burger
            </div>

            <div className="absolute w-[152px] h-6 top-[143px] left-[562px]">
              <Image
                className="absolute w-6 h-6 top-0 left-0"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-8"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-16"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-24"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-23.svg"
                width={24}
                height={24}
              />

              <Image
                className="absolute w-6 h-6 top-0 left-32"
                alt="Frame"
                src="https://c.animaapp.com/89fS0TNm/img/frame-24.svg"
                width={24}
                height={24}
              />
            </div>

            <p className="absolute w-[485px] top-[191px] left-[562px] [font-family:'Nunito',Helvetica] font-normal text-white text-lg tracking-[0] leading-[29.2px]">
              Experience the perfect blend of premium ingredients and expert
              craftsmanship in every bite.
            </p>

            <p className="absolute w-[105px] top-[263px] left-[559px] [font-family:'Inter',Helvetica] font-normal text-[#ebbd28] text-3xl tracking-[0] leading-9 whitespace-nowrap">
              <span className="font-black">₹19.99</span>
            </p>
          </div>

          <div className="absolute w-[156px] h-[60px] top-[252px] left-[907px] bg-[#ebbd28] rounded-[26843500px]">
            <div className="absolute w-[95px] top-[15px] left-8 [font-family:'Nunito',Helvetica] font-bold text-black text-lg text-center tracking-[0] leading-7 whitespace-nowrap">
              Order Now
            </div>
          </div>

          <div className="absolute w-[243px] h-[11px] top-[114px] left-[574px]">
            <div className="absolute w-3 h-[11px] top-0 left-[115px] bg-[#ebbd28] rounded-[5.96px/5.5px]" />

            <Image
              className="left-0 absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-1-3.svg"
              width={95}
              height={1}
            />

            <Image
              className="left-[148px] absolute w-[95px] h-px top-1.5"
              alt="Line"
              src="/img/line-2-4.svg"
              width={95}
              height={1}
            />
          </div>
        </div>
      </div>
    </div>
  );
};
